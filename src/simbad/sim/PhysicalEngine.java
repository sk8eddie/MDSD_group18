...
public class PhysicalEngine {
				...
				for (int j = i + 1; j < nagents; j++) {
					...
					if ((a1.positionChanged) || (a2.positionChanged)) {
						...
						if (bs1.intersect(bs2)) {
							...
							if (checkCollision) {
								// collision indicator.
								// If one of them can be traversed there is no
								// collision.
	 ...
	 * and/or collision.
	 */
	protected void checkAgentObjectPairs(ArrayList agents, ArrayList objects, boolean computeInteraction,
		...
		for (int i = 0; i < nagents; i++) {
			...
			for (int j = 0; j < nobjs; j++) {
				...
				if (o instanceof HorizontalWall || o instanceof VerticalWall) {
	...
	/** experimental */
	private void computeAgentAgentImpact(SimpleAgent a1, SimpleAgent a2, BoundingSphere bs1, BoundingSphere bs2) {
		Vector3d n = v1;
		Vector3d vrel = v2;
	...
	private void computeAgentObjectImpact(SimpleAgent a, StaticObject o, BoundingSphere bsa) {
		...
		if (ndotva <= 0) {
			...
			// System.out.println("add " + v2.toString() +" normal
			// "+n.toString());
	...
	protected double projNormal(double x1, double y1, double z1, double x2, double y2, double z2, double x3, double y3,
			double z3) {
